...
| H-03 | Production settings could start with unsafe development defaults. | Code inspection confirmed that a disabled-debug process could otherwise inherit a default secret, SQLite, or a non-delivering email backend. | Startup now requires an explicit long secret, explicit non-wildcard hosts, PostgreSQL credentials, a delivering email backend, default sender, and encrypted SMTP when SMTP is selected. Invalid numeric and mutually exclusive email settings also fail closed. | `test_production_settings_fail_closed_for_missing_or_unsafe_values`; production container `check --deploy` reported no issues. |
...
| M-02 | Password reset addresses were not unique case-insensitively, and reset requests could flood a known mailbox. | Added a conditional case-insensitive database uniqueness constraint and HMAC-keyed email and IP reset throttling with the same generic success response. Covered by email collision, delivery, non-disclosure, and rate-limit tests. |
...
- case-insensitive password-reset email uniqueness;
- password-reset delivery, non-disclosure, and rate limiting;
...
8. Approved SMTP provider, sender domain, delivery monitoring, account recovery support, and optional SSO or MFA requirements.
